...
import { memoryPromptBlock, type InvestigationMemory } from "../context/memory.js";
import { getLlm } from "../llm/client.js";
import { ANSWER_LANGUAGE_RULE } from "./gate.js";
import { toolDatabase, type Citation } from "./citations.js";
import { ThoughtFlusher } from "./thought-flush.js";
import { loadSkills, skillsForMode } from "../skills/loader.js";

export type ComposeChunk = { kind: "text" | "reasoning"; content: string };

const CLUE_MAX = 5500;
const ROWS_PER_BLOCK = 6;
const LIT_CLUES = 6;

function compactRowLine(row: Record<string, unknown>): string {
...
function looksLikeToolId(s: string): boolean {
  return /^[a-z][a-z0-9_]*$/.test(s);
}

function sourceLabel(tool?: string, source?: string): string {
  const fromTool = tool ? toolDatabase(tool) : "";
  const src = (source || "").trim();
  if (src && !looksLikeToolId(src)) return src;
  if (fromTool && !looksLikeToolId(fromTool)) return fromTool;
  return "database";
}

/** Compact clues for the answer writer — not an inventory to copy. */
export function cluesForComposer(
  ...
  for (const a of artifacts.findDbResults()) {
    if (/\[empty_result\]/i.test(a.summary)) continue;
    const blocks = a.payload?.blocks || [];
    if (!blocks.length) continue;
    for (const b of blocks) {
      if (!b.rows.length && !b.shown) continue;
...
export function composeSystemPrompt(citeList: string): string {
...
export async function* composeAnswer(
  ...
  for await (const ev of llm.chatCompletionStream(messages, {
    ...
    if (ev.type === "reasoning" && ev.content) {
      ...
      if (flushed) yield { kind: "reasoning", content: flushed };
      continue;
    }
  ...
  if (rest) yield { kind: "reasoning", content: rest };
  if (!gotText) throw new Error("empty compose stream");
}
