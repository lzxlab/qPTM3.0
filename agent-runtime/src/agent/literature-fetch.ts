...
function pmidCap(args: Record<string, unknown>): number {
 ...
 */
export async function callSearchLiteratureDeep(args: Record<string, unknown>): Promise<QptmToolResult> {
  ...
  if (hasPmids || hasFulltext) return first;

  const papers = papersFromToolResult(first);
  if (!papersNeedAbstracts(papers)) return first;

  let pmids = papers.map((p) => p.pmid).filter(Boolean);
  if (!pmids.length) {
    pmids = extractPmids(
      `${first.summary || ""}\n${JSON.stringify(first.blocks || first.data || {})}`,
      pmidCap(args),
    );
  }
  pmids = [...new Set(pmids)].slice(0, pmidCap(args));
  if (!pmids.length) return first;
