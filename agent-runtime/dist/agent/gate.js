...
export const ANSWER_LANGUAGE_RULE = "Write the user-visible answer in Chinese only if the user's question is in Chinese; otherwise English.";
...
const BIO_KEYWORDS_EN = /\b(proteins?|genes?|kinases?|phospho\w*|ptms?|post-translational|ubiquitin\w*|acetyl\w*|methyl\w*|sites?|uniprot|pathways?|cells?|organism|mutation|disease|cancer|enzymes?|peptides?|amino\s+acids?|dna|rna|chromosome|apoptosis|receptor|antibody|histone|metabolism|signall?ing)\b/i;
...
const COLLECTION_RE = /\bcollect\b|\bcurat(?:e|ing)\b|\bingest\b|qratio|quantitative\s+table|supplementary\s+table|literature\s+collection|data\s+collect|数据收集|文献收集|抽取|入库|定量表|补充表|策展/i;
const ACCESSION_RE = /(?<![A-Za-z0-9_])((?:PXD|IPX|JPST|MSV|PDC)\d+)(?![A-Za-z0-9_])/gi;
const RESOLVE_URLS_RE = /download\s*url|download\s*link|ftp\s*link|raw\s*file|ms\s*url|pride|iprox|jpost|massive|cptac|proteomexchange|resolve[- ]?url|get[- ]?url|下载链接|下载地址|质谱.*下载|原始数据|获取.*链接|解析.*链接/i;
...
export function extractAccessions(text) {
    if (!text)
        return [];
    const seen = new Set();
    const out = [];
...
export function looksLikeResolveUrlsRequest(message) {
    ...
    if (!accessions.length)
        return false;
    if (RESOLVE_URLS_RE.test(text))
        return true;
    const stripped = text.replace(new RegExp(ACCESSION_RE.source, "gi"), " ").replace(/[\s,;:/|=#\-]+/g, "");
    return stripped.length === 0;
}
export function isCollectionUpload(filenames) {
    if (!filenames?.length)
        return false;
    return filenames.some((name) => FULLTEXT_EXT.test(name) || SUPP_EXT.test(name));
}
export function isCollectionRequest(message, entities, uploadFilenames = []) {
    if (isCollectionUpload(uploadFilenames))
        return true;
    const text = (message || "").trim();
    if (looksLikeResolveUrlsRequest(text))
        return true;
    if (COLLECTION_RE.test(text))
        return true;
...
export function shouldSkipInvestigation(mode) {
...
export function ptmResearchSteer(lang) {
    return lang === "zh"
...
export function classifyQueryMode(message, entities, uploadFilenames = []) {
    const m = (message || "").trim();
    if (isCollectionRequest(m, entities, uploadFilenames))
        return "collection";
    if (!m)
        return "research";
