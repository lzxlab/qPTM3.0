...
import { callQptmTool, biomcpGetArticle, listMcpTools, searchLiteratureArticles, webSearch, } from "../mcp/hub.js";
...
import { gapScore, isCallBug, isEmptyToolResult, mayRecallIntent, outcomesSummary, toStepOutcome, } from "./tool-result.js";
...
export async function executeIntent(tool, memory, question, retryOnBug = true) {
    const args = {
        query: question,
        gene: memory.gene || "",
...
function intentArgs(memory, question) {
    return {
        query: question,
        gene: memory.gene || "",
        position: memory.position || 0,
...
Max 6 steps.`;
    ...
    push(lang === "zh" ? "位点与蛋白背景" : "Site context", "site", lang === "zh" ? "确认靶点" : "Target context", available.filter((t) => /search_ptm_sites|get_localization/.test(t)).slice(0, 2));
    if (/kinase|激酶|调控|enzyme|e3/i.test(q)) {
        push(lang === "zh" ? "上游调控" : "Upstream regulation", "kinase", lang === "zh" ? "激酶/酶证据" : "Kinase evidence", available.filter((t) => /get_upstream_enzymes/.test(t)).slice(0, 1));
    }
    if (/condition|定量|fold|treatment/i.test(q)) {
        push(lang === "zh" ? "定量条件" : "Conditions", "condition", lang === "zh" ? "条件倍数" : "Fold changes", available.filter((t) => /get_site_conditions/.test(t)).slice(0, 1));
    }
    if (/function|disease|疾病|drug|药/i.test(q) || steps.length < 2) {
        push(lang === "zh" ? "功能与疾病" : "Function & disease", "function", lang === "zh" ? "功能/疾病" : "Function/disease", available.filter((t) => /get_function_disease|get_drug_ptm/.test(t)).slice(0, 2));
    }
    if (!steps.length) {
        push(lang === "zh" ? "综合检索" : "Broad search", "site", lang === "zh" ? "广搜" : "Broad", available.slice(0, 4));
    }
...
export async function* runBreadthPlanExecute(userMessage, memory, session, skills, catalog, lang, artifacts, citations, state) {
    yield setPhase(session, AgentPhase.breadth, lang === "zh" ? "广搜多个 PTM 数据库" : "Broad database search");
...
export function buildDepthQuery(focus, memory, question) {
...
export async function* runDepthSearch(userMessage, memory, session, lang, artifacts, citations, state, focus, queryOverride) {
    ...
    yield setPhase(session, AgentPhase.depth, lang === "zh" ? "深挖文献与补充证据" : "Depth search: literature & gaps");
...
function supervisorSystemPrompt(lang, skills, memory) {
    const base = lang === "zh"
...
export async function* runSupervisorLoop(userMessage, memory, session, skills, catalog, lang, artifacts, citations, state) {
    const llm = getLlm();
    const messages = [
    ...
    for (let round = 0; round < cfg.drSupervisorMaxRounds && !finished; round++) {
        yield setPhase(session, AgentPhase.supervising, lang === "zh" ? "评估证据，决定下一步" : "Evaluating evidence, choosing next step");
