...
import { callQptmTool, listMcpTools, searchLiteratureArticles, readQptmResource, isWebSearchFailure, webSearch, } from "../mcp/hub.js";
import { loadSkills, skillsForMode } from "../skills/loader.js";
import { getLlm } from "../llm/client.js";
import { mergeCitation } from "./citations.js";
import { classifyQueryMode, detectLang, gateReply, needsLiterature, ptmResearchSteer, ANSWER_LANGUAGE_RULE, } from "./gate.js";
...
const WEB_SEARCH_TOOL = {
    ...
    function: {
        name: "web_search",
        description: "Optional targeted web search. Call only when the user's intent needs timely web context or a mechanistic gap databases did not cover — never a generic PTM site query.",
        parameters: {
            type: "object",
...
async function executeQptmTool(toolHint, question, memory, artifacts, citations, extra) {
...
async function callQptmToolRaw(toolHint, question, memory, extra) {
...
export async function* runQA(userMessage, history, session) {
    ...
    const messages = [
        {
            role: "system",
            content: `You are a qPTM biology expert. Some database results are already collected. Call remaining database tools only if critical evidence is missing; otherwise answer concisely. Never emit protocol markup.
    ...
    for (let round = 0; round < cfg.qaMaxRounds; round++) {
        ...
        if (allowedCalls.length) {
            ...
            for (const item of extraRaw) {
                ...
                const r = commitToolResult(item.tc.name, item.q, memory, artifacts, citations, item.raw.args, item.raw.result);
                used.add(r.tool);
                toolsUsed.push(r.tool);
                messages.push({
                    role: "tool",
