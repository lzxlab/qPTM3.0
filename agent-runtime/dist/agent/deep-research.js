...
import { resolveSessionTarget, resolvedBanner, } from "./resolve-target.js";
import { sanitizeUserVisibleText } from "./protocol.js";
import { AgentPhase, setPhase } from "./phase.js";
import { createDrState, runSupervisorLoop, } from "./dr-research-loop.js";
export async function* runDeepResearch(userMessage, history, session) {
    ...
    if (cannotInvestigateSiteLevel(memory, userMessage)) {
        yield setPhase(session, AgentPhase.synthesis, lang === "zh" ? "说明缺少靶点" : "Need a protein/site first");
        const reply = lang === "zh"
    ...
    yield* runSupervisorLoop(userMessage, memory, session, skills, sourcesCatalog, lang, artifacts, citations, state);
    ...
    if (banner)
        yield { type: "text", content: banner };
    let reportBody = "";
    try {
    ...
    const followUps = await generateFollowUps(userMessage, report, memory, artifacts, "deep_research", state.toolsUsed);
