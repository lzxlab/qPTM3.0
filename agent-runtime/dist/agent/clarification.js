...
import { extractPositionFromText, memoryPromptBlock, mergeEntities, parseEntities, } from "../context/memory.js";
import { getLlm } from "../llm/client.js";
function opt(label, description = "") {
    return { label, description };
}
/** User wants to discover/rank sites — do not ask for a residue number. */
function isSiteDiscoveryIntent(message) {
    return /哪些位点|哪些磷酸化位点|值得研究.{0,8}位点|位点.{0,8}值得研究|研究得[比多].{0,8}位点|位点.{0,8}研究得|热点位点|hotspots?|which sites|sites worth|worth studying|most[- ]studied sites|research(ed)? (a lot|most).{0,20}sites?/i.test(message);
}
/** Question is about one specific residue (kinase/regulation/function of that site). */
function isSingleSiteMechanismIntent(message) {
    if (isSiteDiscoveryIntent(message))
        return false;
    return /哪个激酶|哪些上游激酶|上游激酶|激酶修饰|修饰了|该位点|这个位点|这个残基|which kinase|upstream kinase|phosphorylat/i.test(message);
}
function hasGene(memory) {
...
export function cannotInvestigateSiteLevel(memory, message) {
    return !memory.gene && !memory.uniprot_ac && isSingleSiteMechanismIntent(message);
}
function hasSite(memory) {
    return Boolean(memory.position);
}
function fallbackFreeText(lang) {
    return lang === "zh"
...
function fallbackGeneClarification(lang) {
    if (lang === "zh") {
        return {
...
function fallbackSiteClarification(memory, lang) {
...
function fallbackPriorityClarification(message, memory) {
 ...
 */
function fallbackClarification(message, memory) {
    const lang = detectLang(message);
    if (!hasGene(memory)) {
...
function normalizePayload(raw, message) {
    if (!raw || typeof raw !== "object")
        return null;
    const data = raw;
    ...
    return {
        ...
        intro: String(data.intro ||
            (lang === "zh"
                ...
                : "Before deep research, I’d like to confirm a few points (all optional):")).slice(0, 300),
        fields,
        free_text: {
            label: String(free?.label || (lang === "zh" ? "补充说明（可选）" : "Additional context (optional)")).slice(0, 80),
            placeholder: String(free?.placeholder ||
                (lang === "zh"
                    ? "物种/细胞系、比较条件、疾病背景等"
                    : "Organism/cell line, comparison, disease background…")).slice(0, 160),
        },
 ...
 */
export async function buildDeepResearchClarification(message, memory, roundOpts = {}) {
    const mem = memory || { ...parseEntities(message) };
    const lang = detectLang(message);
...
export function mergeClarification(original, selections, freeText) {
...
export function applyClarificationToMemory(memory, selections, freeText) {
    ...
    for (const t of texts) {
        ...
        if (pos && !memory.position)
            memory.position = pos;
        if (/小鼠|mouse/i.test(t))
            memory.organism = "mouse";
        if (/人源|人类|human/i.test(t) && memory.organism !== "mouse")
            memory.organism = "human";
    }
}
export function clarificationEvent(payload) {
