    ...
    for (let round = 0; round < cfg.reactMaxRounds; round++) {
        ...
        if (calls.length) {
            ...
            for (const item of executed) {
                ...
                const payload = compactDbPayload(result);
                if (empty)
                    emptyKeys.add(emptyCallKey(item.tc.name, item.args));
                addFinding(memory, item.tc.name, `${result.summary}${result.error_kind ? ` [${result.error_kind}]` : ""}`);
                if (!empty) {
                    if (item.tc.name === "search_literature") {
                        ...
                        for (const p of papers) {
                            if (!p.abstract && !p.fulltext)
                                continue;
                            artifacts.add("paper", `PMID:${p.pmid}`, [`PMID:${p.pmid}`, p.title, p.abstract, p.fulltext].filter(Boolean).join("\n").slice(0, cfg.litAbstractMaxChars), { pmids: [p.pmid] });
                        }
                        yield {
                        ...
                        if (result.success)
                            mergeCitation(citations, item.tc.name);
                    }
                }
                if (!toolsUsed.includes(item.tc.name))
                    toolsUsed.push(item.tc.name);
                const entities = followableEntitiesFromPayload(payload, [memory.gene || ""]);
                const observation = formatToolObservation(item.tc.name, result, payload, entities, [memory.gene || ""]);
                messages.push({
                    role: "tool",
                    tool_call_id: item.tc.id,
                    content: observation,
